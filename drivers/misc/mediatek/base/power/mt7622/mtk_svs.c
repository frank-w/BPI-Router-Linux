...
static int limit_mcusys_env(struct svs_det *det)
{
	...
	if (ret) {
		svs_error("cpufreq is not ready.\n");
		return ret;
	}
	...
	if (ret)
		svs_error("%s: Failed to set regulator in PWM mode\n", det->name);
...
static int unlimit_mcusys_env(struct svs_det *det)
{
	...
	if (ret)
		svs_error("%s: Failed to set regulator in normal mode\n", det->name);
...
static inline void handle_init_err_isr(struct svs_det *det)
{
	...
	svs_isr_info("====================================================\n");
#if 0
	aee_kernel_warning("mt_ptp", "@%s():%d, VBOOT(%s) = 0x%08X\n", __func__, __LINE__, det->name, det->VBOOT);
#endif
...
static int svs_probe(struct platform_device *pdev)
{
	...
	if (WARN_ON(ret)) {
		svs_error("fail to cpufreq_register_notifier, ret = %d\n", ret);
		return ret;
	}
	...
	if (ret) {
		svs_error("cpufreq_unregister_notifier (%d)\n", ret);
		WARN_ON(1);
		return ret;
	}
...
static int svs_stress_result_proc_show(struct seq_file *m, void *v)
{
	if (stress_result != 0)
		svs_isr_info("SVS fail to trigger irq\n");
...
static int svs_status_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%s: svs_cpufreq_policy.max = %lu\n", det->name, det->dvfs_max_freq_khz);
	seq_printf(m, "%s: svs_cpufreq_policy.min = %lu\n", det->name, det->dvfs_min_freq_khz);
...
PROC_FOPS_RO(svs_stress_result);
