...
static int mtketh_mt7530sw_debugfs_write(struct file *file, const char __user *ptr,
					 size_t len, loff_t *off)
{
	...
	pr_info("%s:phy=%d, reg=0x%x, val=0x%x\n", __func__,
		0x1f, reg, value);
	mt7530_mdio_w32(gsw, reg, value);
	pr_info("%s:phy=%d, reg=0x%x, val=0x%x confirm..\n", __func__,
		0x1f, reg, mt7530_mdio_r32(gsw, reg));

	return len;
}

static int mtketh_debugfs_write(struct file *file, const char __user *ptr,
				size_t len, loff_t *off)
{
	struct mtk_eth *eth = file->private_data;
	char buf[32], *token, *p = buf;
...
static int mtketh_debugfs_reset(struct file *file, const char __user *ptr,
				size_t len, loff_t *off)
{
	struct mtk_eth *eth = file->private_data;
...
int mtketh_debugfs_init(struct mtk_eth *eth)
{
	...
	if (!eth->debug.root) {
		dev_err(eth->dev, "%s:err at %d\n", __func__, __LINE__);
		ret = -ENOMEM;
	}

	debugfs_create_file("phy_regs", S_IRUGO, eth->debug.root, eth, &mtketh_debug_fops);
	debugfs_create_file("phy_reg_w", S_IFREG | S_IWUSR, eth->debug.root, eth,  &fops_reg_w);
	debugfs_create_file("reset", S_IFREG | S_IWUSR, eth->debug.root, eth,  &fops_eth_reset);
	if (eth->switch_np && eth->sw_priv) {
		debugfs_create_file("mt7530sw_regs", S_IRUGO, eth->debug.root, eth,  &mtketh_debug_mt7530sw_fops);
		debugfs_create_file("mt7530sw_reg_w", S_IFREG | S_IWUSR, eth->debug.root, eth,  &fops_mt7530sw_reg_w);
	}
	return ret;
}
...
int mtk_do_priv_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case MTKETH_ESW_REG_READ:
		...
		if (reg.off > REG_ESW_MAX)
			return -EINVAL;
		reg.val = mtk_switch_r32(gsw, reg.off);
	...
	case MTKETH_ESW_REG_WRITE:
		...
		if (reg.off > REG_ESW_MAX)
			return -EINVAL;
		mtk_switch_w32(gsw, reg.val, reg.off);
...
int esw_cnt_read(struct seq_file *seq, void *v)
{
	...
	struct mt7620_gsw *__gsw = (struct mt7620_gsw *)eth->sw_priv;
...
int tx_ring_read(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "cpu last free: %d\n", ring->last_free - ring->dma);
	for (i = 0; i < MTK_DMA_SIZE; i++) {
		seq_printf(seq, "%d (%08x): %08x %08x %08x %08x\n", i,
			   ring->phys + i * sizeof(*tx_ring),
...
{
	struct mtk_rx_ring *ring = &g_eth->rx_ring;
	...
	seq_printf(seq, "next to read: %d\n", NEXT_RX_DESP_IDX(ring->calc_idx));
	for (i = 0; i < MTK_DMA_SIZE; i++) {
