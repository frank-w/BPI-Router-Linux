...
// ============================================ RTK VLAN functions ==========================================================
static ret_t rtl8367c_resetVlan(void)
{
    ret_t   retVal;
...
static rtk_api_ret_t rtk_vlan_reset(void)
{
    rtk_api_ret_t retVal;
...
static rtk_api_ret_t rtk_vlan_init(void)
{
    rtk_api_ret_t retVal;
    rtk_uint32 i;
...
static enum dsa_tag_protocol rtl8367c_get_tag_protocol(struct dsa_switch *ds,
						      int port,
...
static int rtl8367c_setup_mdio(struct rtk_gsw *gsw)
{
...
/*	.phylink_mac_link_up = rtl8367c_mac_link_up,
	.phylink_mac_link_down = rtl8367c_mac_link_down,
	.get_strings = rtl8367c_get_strings,
	.get_ethtool_stats = rtl8367c_get_ethtool_stats,
	.get_sset_count = rtl8367c_get_sset_count,
	.port_vlan_filtering = rtl8367c_vlan_filtering,
	.port_vlan_add = rtl8367c_vlan_add,
	.port_vlan_del = rtl8367c_vlan_del,
	.port_enable = rtl8367c_port_enable,
	.port_disable = rtl8367c_port_disable,
	.port_change_mtu = rtl8367c_change_mtu,
	.port_max_mtu = rtl8367c_max_mtu,*/
};
...
static int rtk_gsw_probe(struct platform_device *pdev)
{
	...
	#ifdef RTL8367_USE_DSA
	//start dsa initialization
	gsw->ds = devm_kzalloc(dev, sizeof(*gsw->ds), GFP_KERNEL);
	if (!gsw->ds)
		return -ENOMEM;

	gsw->ds->dev = &pdev->dev;
	gsw->ds->num_ports = RTK_SWITCH_PORT_NUM;//gsw->num_ports;
	gsw->ds->priv = gsw;

	gsw->ds->ops = &rtl8367c_switch_ops;

	ret = dsa_register_switch(gsw->ds);
	if (ret) {
		dev_err(&pdev->dev, "unable to register switch ret = %d\n", ret);
		//return ret; //this may break phy driver if register switch failes
	}
	#endif
