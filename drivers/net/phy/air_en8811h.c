...
static umode_t air_hwmon_is_visible(const void *data,
...
static int en8811h_hwmon_read(struct device *dev,
			      enum hwmon_sensor_types type,
			      u32 attr, int channel, long *value)
{
	...
	*value = pbus_value;
	...
	phydev_info(phydev, "EEEEEEEEEEEEEEEEEEEEEEE %d\n", pbus_value);

	return 0;
}

static const struct hwmon_channel_info * const en8811h_hwmon_info[] = {
	HWMON_CHANNEL_INFO(temp, HWMON_T_INPUT),
	NULL
};

static const struct hwmon_ops en8811h_hwmon_hwmon_ops = {
	.is_visible	= air_hwmon_is_visible,
	.read		= en8811h_hwmon_read,
};

static const struct hwmon_chip_info en8811h_hwmon_chip_info = {
	.ops		= &en8811h_hwmon_hwmon_ops,
	.info		= en8811h_hwmon_info,
};

static int en8811h_hwmon_register(struct phy_device *phydev)
{
	struct device *dev = &phydev->mdio.dev;
...
static int en8811h_probe(struct phy_device *phydev)
{
	...
	return en8811h_hwmon_register(phydev);
}
