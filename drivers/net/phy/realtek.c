...
static int rtl822x_c45_get_rate_matching(struct phy_device *phydev,
...
static int rtl822x_c45_config_aneg(struct phy_device *phydev)
{
	...
	if (linkmode_test_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT,
			phydev->advertising))
		val |= RTL822x_V2_GBCR_1000BASET_FULL_DUPLEX_CAP;
	ret =  phy_modify_mmd_changed(phydev, MDIO_MMD_VEND2, RTL822x_V2_GBCR,
			       RTL822x_V2_GBCR_1000BASET_FULL_DUPLEX_CAP, val);
	if (ret < 0)
		return ret;
	if (ret > 0)
		changed = true;
...
static struct phy_driver realtek_drvs[] = {
	{
		...
		.get_rate_matching = rtl822x_c45_get_rate_matching,
