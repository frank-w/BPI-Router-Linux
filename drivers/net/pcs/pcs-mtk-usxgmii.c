...
{
	struct mtk_sgmii_wrapper_pcs *wp = container_of(mpcs->sgmii_pcs, struct mtk_sgmii_wrapper_pcs,
							pcs);
				      ...
				      struct phylink_link_state *state)
{
	...
	usxgmii_an = FIELD_GET(USXGMII_AN_ENABLE, mtk_r32(mpcs, RG_PCS_AN_CTRL0));
	if (usxgmii_an)
