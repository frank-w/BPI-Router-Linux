...
static int fitblk_open(struct gendisk *disk, fmode_t mode)
{
	...
	if (fitblk->lower_bdev->bd_disk->fops->open)
		return fitblk->lower_bdev->bd_disk->fops->open(fitblk->lower_bdev->bd_disk, mode);
	else
		return 0;
}
...
static void fitblk_release(struct gendisk *disk)
{
	...
	if (fitblk->lower_bdev->bd_disk->fops->release)
		fitblk->lower_bdev->bd_disk->fops->release(fitblk->lower_bdev->bd_disk);
}
...
static void fitblk_mark_dead(struct block_device *bdev)
{
...
static int parse_fit_on_dev(struct device *dev)
{
	...
	bdev = blkdev_get_by_dev(dev->devt, FMODE_READ, &_fitblk_claim_ptr, &fitblk_hops);
	if (!bdev)
		return -ENODEV;
