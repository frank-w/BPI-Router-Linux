...
static void fitblk_submit_bio(struct bio *orig_bio)
{
	...
	while (bio) {
		...
		bio->bi_bdev = fitblk->lower_bdh->bdev;
		bio = bio->bi_next;
	}
	submit_bio(orig_bio);
}
...
static void fitblk_purge(struct work_struct *work)
{
	...
	if (refcount_dec_if_one(&num_devs)) {
		...
		bdev_release(fitblk->lower_bdh);
	}

	kfree(fitblk);
}

static int add_fit_subimage_device(struct bdev_handle *lower_bdh,
				   ...
				   sector_t nr_sect, bool readonly)
{
	...
	disk->first_minor = 0;
	disk->flags = lower_bdh->bdev->bd_disk->flags | GENHD_FL_NO_PART;
	...
	disk->queue->queue_flags = lower_bdh->bdev->bd_disk->queue->queue_flags;
	memcpy(&disk->queue->limits, &lower_bdh->bdev->bd_disk->queue->limits,
...
static int parse_fit_on_dev(struct device *dev)
{
	...
	bdh = bdev_open_by_dev(dev->devt, FMODE_READ, &_fitblk_claim_ptr, &fitblk_hops);
	if (!bdh)
		return -ENODEV;
	...
	mapping = bdh->bdev->bd_inode->i_mapping;
	...
	/* acquire disk size */
	dsectors = bdev_nr_sectors(bdh->bdev);
	...
	fdt_for_each_subnode(node, fit, images) {
		...
		if (!slot) {
			ret = sysfs_create_link_nowarn(&pdev->dev.kobj, bdev_kobj(bdh->bdev), "lower_dev");
			if (ret && ret != -EEXIST)
				goto out_bootconf;
		...
		add_fit_subimage_device(bdh, slot++, start_sect, nr_sects, true);
	}
	...
	if (!bdh->bdev->bd_read_only && bdev_is_partition(bdh->bdev) &&
	    (imgmaxsect + MIN_FREE_SECT) < dsectors) {
		add_fit_subimage_device(bdh, slot++, imgmaxsect,
