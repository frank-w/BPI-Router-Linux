...
static int parse_fit_on_dev(struct device *dev)
{
	...
	bdev_file = bdev_file_open_by_dev(dev->devt, BLK_OPEN_READ | BLK_OPEN_RESTRICT_WRITES,
	...
	fit_c = fit;
	while (bytes_left > 0) {
		bytes_to_copy = min_t(size_t, bytes_left, folio_size(folio) - offset_in_folio(folio, 0));
